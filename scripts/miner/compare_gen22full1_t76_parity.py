...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--positive-votes-required", type=int, default=76)
...
def main() -> int:
    ...
    for task_index, entry in enumerate(log_entries, start=1):
        ...
        for chunk_index, chunk in enumerate(chunks):
            ...
            matches = abs(float(miner_score) - float(standalone["score"])) < 1e-9 and miner_pred == bool(standalone["prediction"])
    ...
    out_path = Path(args.out).expanduser().resolve() if args.out else REPO_ROOT / "models" / "gen22full1_parity_check.json"
